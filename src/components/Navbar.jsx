...
import { Github, Plus, ChevronDown, Bell, Moon, Sun, Sparkles, LogOut } from 'lucide-react';
    ...
    <nav className="w-full bg-github-header border-b border-github-border text-github-textPrimary sticky top-0 z-50">
      <div className="max-w-[1400px] mx-auto px-4 py-3 sm:px-6 flex items-center justify-between">
        ...
        <div className="flex items-center space-x-4">
          <Link to="/" className="text-github-textPrimary hover:text-github-blue transition-colors">
            <Github className="w-8 h-8" />
          </Link>
          <div className="hidden md:flex space-x-2">
            <div className="flex bg-github-canvas border border-github-border rounded-md px-2 py-1 items-center min-w-[240px]">
              <span className="text-github-textMuted text-xs font-mono ml-2">Type / to search</span>
            </div>
          </div>
        ...
        <div className="flex items-center space-x-3 text-github-textPrimary font-semibold">
          {!loading && !user ? (
            ...
            <div className="flex items-center space-x-2">
              <Link to="/login" className="btn-outline border-transparent text-sm px-3">
                Sign In
              </Link>
            ...
            // Loading state
            <div className="animate-pulse text-github-textMuted text-sm">Loading...</div>
          ) : (
              ...
              {showUserMenu && (
                <div className="absolute right-0 mt-2 w-48 rounded-md shadow-lg bg-github-canvas border border-github-border overflow-hidden">
                  <div className="px-4 py-3 border-b border-github-border">
                    <div className="text-sm font-semibold text-github-textPrimary truncate">{user?.email}</div>
                  </div>
                  <Link
                    to="/profile"
            ...
            {showThemeMenu && (
              <div className="absolute right-0 mt-2 w-48 rounded-md shadow-lg bg-github-canvas border border-github-border overflow-hidden">
                ...
                >
                  ...
                  <span>Light</span>
                  {theme === 'github-light' && <span className="ml-auto text-github-blue">✓</span>}
                </button>
                <button
                  onClick={() => {
                ...
                >
                  ...
                  <span>Dark</span>
                  {theme === 'github-dark' && <span className="ml-auto text-github-blue">✓</span>}
                </button>
                <button
                  onClick={() => {
                ...
                >
                  ...
                  {theme === 'celestis' && <span className="ml-auto text-github-blue">✓</span>}
                </button>
              </div>
            )}
          </div>
      ...
      <div className="bg-github-canvas border-b border-github-border pt-4">
        <div className="max-w-[1400px] mx-auto px-4 sm:px-6">
          <div className="flex items-center text-xl pb-4">
            <span className="text-github-blue hover:underline cursor-pointer">Celestis</span>
            <span className="mx-2 text-github-textMuted font-light">/</span>
            <span className="text-github-blue hover:underline cursor-pointer font-bold">Open-Source-Week</span>
